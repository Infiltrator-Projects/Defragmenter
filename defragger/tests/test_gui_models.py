...
def test_ui_polish_preserves_allocation_map_visual_contract() -> None:
    ...
    for required in (
        ...
        "Gtk.Stack()",
        'self._show_page("analyse")',
        'self._show_page("defrag")',
        'self._show_page("growth")',
        'self._show_page("recover")',
        'self._show_page("test-media")',
...
def test_main_window_remains_resizable_maximisable_and_workarea_bounded() -> None:
    ...
    assert "self.volumes.discover()" in window_source
    assert "self.volumes.apply_discovery(" in window_source
