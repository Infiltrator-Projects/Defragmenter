...
def test_infiltratr_common_integration() -> None:
    ...
    assert (common / "VERSION").read_text().strip() == "1.19.22"
    gitmodules = (ROOT / ".gitmodules").read_text()
    assert "shared/infiltratr-common" in gitmodules
    assert "Infiltrator-Libraries.git" in gitmodules
    cmake = _cmake_source()
    assert "302c44eb7436803dee020667453a9a0681da8bbf" in cmake
    ...
    assert 'COMMON_VERSION="1.19.22"' in local_installer
    assert 'COMMON_COMMIT="302c44eb7436803dee020667453a9a0681da8bbf"' in local_installer
    ...
    for role in (
        ...
        assert f"palette->{role}" in test_media_theme, (
            f"Test Media does not consume Common 1.19.22 role {role}"
        )
