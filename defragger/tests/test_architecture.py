...
def test_infiltratr_common_integration() -> None:
    ...
    assert (common / "VERSION").read_text().strip() == "1.19.23"
    gitmodules = (ROOT / ".gitmodules").read_text()
    assert "shared/infiltratr-common" in gitmodules
    assert "Infiltrator-Libraries.git" in gitmodules
    cmake = _cmake_source()
    assert "a9cf2957cffeefe6001830916b8a32c2ef58a551" in cmake
    ...
    assert 'COMMON_VERSION="1.19.23"' in local_installer
    assert 'COMMON_COMMIT="a9cf2957cffeefe6001830916b8a32c2ef58a551"' in local_installer
    ...
    for role in (
        ...
        assert f"palette->{role}" in test_media_theme, (
            f"Test Media does not consume Common 1.19.23 role {role}"
        )
