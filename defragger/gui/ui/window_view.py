...
class WindowView:
    ...
    def show_selected_volume(self, selected: str | None) -> None:
        """Synchronise every selected-volume identity surface from one value."""

        if selected:
            parts = [part.strip() for part in selected.split("—")]
            title = parts[0] if parts else selected
            ...
            # the substring "mounted" inside "unmounted" can never win.
            lowered = selected.casefold()
            if ", unmounted" in lowered:
                self.hero_status.set_text("● Offline")
            elif ", mounted" in lowered:
                self.hero_status.set_text("● Mounted")
            else:
                self.hero_status.set_text("● Status unknown")
            return
    ...
    def populate_volumes(self, names: list[str], active_index: int) -> None:
        ...
        self.show_selected_volume(
            names[active_index] if 0 <= active_index < len(names) else None
        )
