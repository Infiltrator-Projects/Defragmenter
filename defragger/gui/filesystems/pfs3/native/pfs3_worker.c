...
#define PROG "linux-defragger-pfs3-worker"
#define JOURNAL_MAGIC "LINUX-DEFRAGGER-PFS3-JOURNAL-1"
#define STOPPED 130
#define PFS3_WORKER_IO_BYTES (1024U * 1024U)
...
static int analyse(const char *path, Pfs3Analysis *analysis)
{
    char error[512] = {0};
    if (pfs3_analyse(path, analysis, NULL, 0U, error, sizeof(error)) != 0) {
        (void)fprintf(stderr, "%s: %s\n", PROG,
                      error[0] != '\0' ? error : "PFS3 analysis failed");
        return -1;
    }
    return 0;
}

static const char *format_name(const Pfs3Analysis *analysis)
{
    (void)analysis;
    return "PFS3";
}

static void print_identify(const Pfs3Analysis *analysis)
{
    (void)printf("{\"filesystem\":\"pfs3\",\"format\":\"%s\"}\n",
                 format_name(analysis));
}

static void print_analysis_json(const Pfs3Analysis *analysis)
{
    (void)printf(
        "{\"filesystem\":\"pfs3\",\"format\":\"%s\","
...
static int print_map(const char *path, uint64_t requested_cells)
{
    ...
    if (cells > SIZE_MAX ||
        !infiltratr_size_multiply_checked((size_t)cells,
                                          sizeof(Pfs3MapCell), &map_bytes)) {
        (void)fprintf(stderr, "%s: allocation map is too large\n", PROG);
        return -1;
    }
    (void)map_bytes;
    Pfs3MapCell *map = calloc((size_t)cells, sizeof(*map));
    if (map == NULL) {
        (void)fprintf(stderr, "%s: out of memory building PFS3 allocation map\n", PROG);
        return -1;
    }
    Pfs3Analysis analysis;
    char error[512] = {0};
    if (pfs3_analyse(path, &analysis, map, cells, error, sizeof(error)) != 0) {
        (void)fprintf(stderr, "%s: %s\n", PROG,
                      error[0] != '\0' ? error : "PFS3 allocation scan failed");
        free(map);
        return -1;
    }
    ...
    (void)printf(
        "{\"schema\":1,\"backend\":\"read-only-domain\",\"filesystem\":\"pfs3\","
    ...
    (void)printf(
        ...
        "\"primary_root_valid\":%s,\"backup_root_valid\":%s,"
        "\"transaction_pending\":%s,\"fragmentation_available\":true,"
        "\"fragmentation_basis\":\"validated PFS3 directory entries and anode chains\","
        "\"allocation_basis\":\"validated PFS3 bitmap-index and allocation-bitmap hierarchy\"}}\n",
        format_name(&analysis),
        analysis.structure_version, analysis.sequence_number,
...
static void journal_free(Pfs3Journal *state)
{
    if (state == NULL)
        return;
    free(state->device);
...
static int ensure_directory_tree(const char *path, char *error, size_t error_size)
{
    if (ld_path_ensure_trusted_directory_tree(path) != 0) {
        worker_error(error, error_size,
                     "cannot create PFS3 journal directory %s: %s",
                     path, strerror(errno));
        return -1;
    }
    return 0;
...
{
    const Pfs3Journal *state = user_data;
    (void)fprintf(file, "%s\n", JOURNAL_MAGIC);
...
static int journal_save(const char *path, const Pfs3Journal *state,
                        char *error, size_t error_size)
{
    if (!safe_value(state->device) || !safe_value(state->target_identity) ||
        !safe_value(state->stage)) {
        worker_error(error, error_size,
                     "PFS3 transaction paths contain unsupported journal characters");
        return -1;
    }
    ...
    if (failure != 0) {
        worker_error(error, error_size,
                     "cannot publish PFS3 recovery journal: %s",
                     strerror(failure));
        return -1;
    }
    return 0;
...
static int journal_load(const char *path, Pfs3Journal *state,
                        char *error, size_t error_size)
{
    memset(state, 0, sizeof(*state));
    ...
    if (file == NULL) {
        worker_error(error, error_size,
                     "cannot open PFS3 recovery journal: %s", strerror(errno));
        return -1;
    }
    char *line = NULL;
    size_t capacity = 0U;
...
invalid_state:
    journal_free(state);
    worker_error(error, error_size,
                 "PFS3 recovery journal is malformed or incomplete");
    return -1;
}

static int journal_phase(const char *path, Pfs3Journal *state,
...
static void transaction_cleanup(const char *journal, const Pfs3Journal *state)
{
    if (state != NULL)
...
{
    return ld_path_append_suffix(journal, ".pfs3-stage");
}
...
static char *canonical_path(const char *path, char *error, size_t error_size)
{
    ...
    if (resolved == NULL)
        worker_error(error, error_size,
                     "cannot resolve PFS3 target %s: %s", path, strerror(errno));
    return resolved;
}
                           ...
                           char *error, size_t error_size)
{
    ...
    if (stat(path, &status) != 0) {
        worker_error(error, error_size,
                     "cannot stat PFS3 target: %s", strerror(errno));
        return -1;
    }
    if (!S_ISBLK(status.st_mode) && !S_ISREG(status.st_mode)) {
        worker_error(error, error_size,
                     "PFS3 target is not a block device or regular image");
        return -1;
    }
    char text[160];
    ...
    if (*size == 0U) {
        worker_error(error, error_size, "cannot determine PFS3 target size");
        return -1;
    }
                            ...
                            char *error, size_t error_size)
{
    ...
    if (EVP_DigestFinal_ex(context, digest, &length) != 1 || length != 32U) {
        worker_error(error, error_size, "finalising PFS3 SHA-256 failed");
        return -1;
    }
                       ...
                       char output[65], char *error, size_t error_size)
{
    ...
    if (fd < 0) {
        worker_error(error, error_size,
                     "cannot open PFS3 data for SHA-256: %s", strerror(errno));
        return -1;
    }
    EVP_MD_CTX *context = EVP_MD_CTX_new();
    uint8_t *buffer = malloc(PFS3_WORKER_IO_BYTES);
    if (context == NULL || buffer == NULL ||
        EVP_DigestInit_ex(context, EVP_sha256(), NULL) != 1) {
        free(buffer);
        EVP_MD_CTX_free(context);
        (void)close(fd);
        worker_error(error, error_size, "initialising PFS3 SHA-256 failed");
        return -1;
    }
    int rc = 0;
    for (uint64_t offset = 0U; offset < bytes;) {
        ...
        const size_t count = remaining > PFS3_WORKER_IO_BYTES ?
                             PFS3_WORKER_IO_BYTES : (size_t)remaining;
        if (ld_pread_full(fd, buffer, count, offset) != (ssize_t)count ||
            EVP_DigestUpdate(context, buffer, count) != 1) {
            worker_error(error, error_size,
                         "hashing PFS3 bytes at offset %" PRIu64 " failed", offset);
            rc = -1;
            break;
        }
        offset += count;
                      ...
                      char *error, size_t error_size)
{
    if (block_size == 0U || total_blocks < 2U) {
        worker_error(error, error_size, "invalid PFS3 root-token geometry");
        return -1;
    }
    int fd = open(path, O_RDONLY | O_CLOEXEC);
    ...
    if (fd < 0 || block == NULL || context == NULL ||
        ...
        free(block);
        EVP_MD_CTX_free(context);
        worker_error(error, error_size, "cannot initialise PFS3 root identity");
        return -1;
    }
    int rc = 0;
    if (ld_pread_full(fd, block, block_size, 0U) != (ssize_t)block_size ||
        ...
        ld_pread_full(fd, block, block_size,
            ...
            (ssize_t)block_size ||
        EVP_DigestUpdate(context, block, block_size) != 1) {
        worker_error(error, error_size, "cannot read PFS3 redundant roots for identity");
        rc = -1;
    }
    if (rc == 0)
        rc = digest_final_hex(context, output, error, error_size);
...
static int capture_target(const char *device, Pfs3Journal *state,
                          char *error, size_t error_size)
{
    ...
    if (target_identity(state->device, &state->target_identity,
        ...
        return -1;
    Pfs3Analysis analysis;
    if (pfs3_analyse(state->device, &analysis, NULL, 0U,
                    error, error_size) != 0)
        return -1;
    if (analysis.transaction_pending) {
        worker_error(error, error_size,
                     "PFS3 filesystem has an unfinished native transaction marker");
        return -1;
    }
    ...
    if (state->filesystem_bytes > state->physical_bytes) {
        worker_error(error, error_size, "PFS3 filesystem exceeds target capacity");
        return -1;
    }
...
static int check_target_identity(const char *device, const Pfs3Journal *state,
                                 char *error, size_t error_size)
{
    ...
    if (rc == 0 &&
        ...
        worker_error(error, error_size,
                     "PFS3 target path, identity or capacity changed before commit");
        rc = -1;
    }
    char token[65];
    if (rc == 0 &&
        ...
        if (error[0] == '\0')
            worker_error(error, error_size,
                         "PFS3 redundant-root identity changed before commit");
        rc = -1;
    }
    free(identity);
    free(canonical);
...
static int check_source_unchanged(const char *device, const Pfs3Journal *state,
                                  char *error, size_t error_size)
{
    ...
    if (strcmp(digest, state->source_sha256) != 0) {
        worker_error(error, error_size,
                     "PFS3 source changed after preflight; refusing source writes");
        return -1;
    }
    return 0;
}

static int stage_sha256(const char *stage, const Pfs3Journal *state,
                        char output[65], char *error, size_t error_size)
{
    Pfs3Analysis analysis;
    ...
    if (analysis.filesystem_bytes != state->filesystem_bytes ||
        ...
        worker_error(error, error_size,
                     "verified PFS3 stage geometry differs from source");
        return -1;
    }
...
static int safe_commit_stage(const char *stage_path, const char *target_path,
                             const Pfs3Journal *state, uint64_t *written,
                             char *error, size_t error_size)
{
    Pfs3Analysis analysis;
    if (pfs3_analyse(stage_path, &analysis, NULL, 0U,
                    error, error_size) != 0)
        return -1;
    if (analysis.filesystem_bytes != state->filesystem_bytes) {
        worker_error(error, error_size, "PFS3 stage size changed before commit");
        return -1;
    }
    ...
    if (stage < 0 || target < 0) {
        ...
        worker_error(error, error_size,
                     "cannot open PFS3 stage or source for commit: %s",
                     strerror(errno));
        return -1;
    }
    if (flock(target, LOCK_EX | LOCK_NB) != 0) {
        worker_error(error, error_size,
                     "cannot lock PFS3 source for commit: %s", strerror(errno));
        (void)close(stage);
        (void)close(target);
        return -1;
    }
    ...
    if (buffer == NULL) {
        worker_error(error, error_size,
                     "out of memory batching PFS3 source commit");
    ...
    for (uint64_t offset = 0U; offset < state->filesystem_bytes;) {
        ...
        if (ld_pread_full(stage, buffer, count, offset) != (ssize_t)count ||
            ...
            worker_error(error, error_size,
                         "short I/O committing PFS3 bytes at offset %" PRIu64,
                         offset);
            rc = -1;
            break;
        }
                           ...
                           bool live, char *error, size_t error_size)
{
    ...
    if (journal_load(journal, &state, error, error_size) != 0)
        return 1;
    if (!ld_path_is_derived_from(state.stage, journal, ".pfs3-stage")) {
        worker_error(error, error_size,
                     "PFS3 recovery stage is not derived from the selected journal path");
        journal_free(&state);
        return 1;
    }
    if (!valid_operation(state.operation) || !valid_phase(state.phase)) {
        worker_error(error, error_size,
                     "PFS3 recovery journal contains an unsupported operation or phase");
        journal_free(&state);
        return 1;
    }
    ...
    if (digest_rc != 0 || strcmp(digest, state.stage_sha256) != 0) {
        if (digest_rc == 0)
            worker_error(error, error_size,
                         "PFS3 recovery stage SHA-256 does not match the journal");
        journal_free(&state);
        return 1;
    }
    const bool growth = strcmp(state.operation, "growth-defrag") == 0;
    if (pfs3_verify_layout(state.stage, growth, 10U,
                          error, error_size) != 0) {
        journal_free(&state);
        return 1;
    }
    if (strcmp(state.phase, "committed") == 0) {
        if (pfs3_verify_layout(device, growth, 10U,
                              error, error_size) != 0) {
            journal_free(&state);
            return 1;
        }
        ...
        ld_emit_result_event(stdout, "recover", "completed",
               "Verified an already committed PFS3 transaction.");
        journal_free(&state);
        return 0;
    }
    ...
    if (commit_rc != 0 ||
        pfs3_verify_layout(state.device, growth, 10U,
                          error, error_size) != 0) {
        journal_free(&state);
        return 1;
    }
    ...
    transaction_cleanup(journal, &state);
    if (live) {
        (void)printf("@@LIVE_RESET {\"reason\":\"authoritative post-recovery PFS3 map\"}\n");
        (void)fflush(stdout);
    }
    (void)printf("Recovered verified PFS3 source; committed %" PRIu64 " KiB.\n",
                 written / 1024U);
    ld_emit_result_event(stdout, "recover", "completed", "");
    journal_free(&state);
    return 0;
...
int main(int argc, char **argv)
{
    ...
    for (int index = 3; index < argc; ++index) {
        if (strcmp(argv[index], "--write") == 0) {
            ...
            (void)fprintf(stderr, "%s: unknown or incomplete PFS3 option: %s\n",
                          PROG, argv[index]);
            return 2;
        }
    }
    if (!write || confirm == NULL || journal == NULL ||
        ...
        (void)fprintf(stderr,
                      "%s: PFS3 mutation requires --write --confirm DEVICE --journal PATH\n",
                      PROG);
        return 2;
    }
    if (growth && growth_percent != 10U) {
        (void)fprintf(stderr,
                      "%s: PFS3 Growth Defrag requires exactly 10 percent reserve\n",
                      PROG);
        return 2;
    }
    if (ld_path_is_mounted(device)) {
        (void)fprintf(stderr,
                      "%s: PFS3 target is mounted; raw mutation and recovery require an unmounted filesystem\n",
                      PROG);
        return 1;
    }
    ...
    if (recover) {
        ...
        if (rc != 0 && rc != STOPPED)
            (void)fprintf(stderr, "%s: %s\n", PROG,
                          error[0] != '\0' ? error : "PFS3 recovery failed");
        return rc;
    }
    ...
    if (capture_rc == STOPPED) {
        ld_emit_result_event(stdout, mode, "stopped", "Stopped during read-only PFS3 preflight.");
        journal_free(&state);
        return STOPPED;
    }
    if (capture_rc != 0)
    ...
    if (state.stage == NULL) {
        worker_error(error, sizeof(error), "out of memory creating PFS3 stage path");
        goto fail;
    }
    if (access(journal, F_OK) == 0 || access(state.stage, F_OK) == 0) {
        worker_error(error, sizeof(error),
                     "existing PFS3 recovery artifacts must be recovered or removed before starting a new transaction");
        goto fail;
    }

    (void)printf("Starting native C PFS3 %s on %s.\n",
                 growth ? "Growth Defrag" : "Defrag", device);
    uint64_t planned = 0U;
    if (pfs3_build_stage(state.device, state.stage, growth, growth_percent,
                        live, &planned, error, sizeof(error)) != 0 ||
        pfs3_verify_layout(state.stage, growth, growth_percent,
    ...
    if (ld_stop_requested()) {
        unlink_if_exists(state.stage);
        ld_emit_result_event(stdout, mode, "stopped", "Stopped before any PFS3 source writes.");
        journal_free(&state);
        return STOPPED;
    }
    ...
    (void)printf("PFS3 source commit: replaying %" PRIu64
    ...
    if (commit_rc == STOPPED) {
        ld_emit_result_event(stdout, mode, "stopped",
               "Run Recover to resume the verified PFS3 transaction.");
        journal_free(&state);
        return STOPPED;
    }
    if (commit_rc != 0 ||
        pfs3_verify_layout(state.device, growth, growth_percent,
                          error, sizeof(error)) != 0)
        goto fail;
    ...
    transaction_cleanup(journal, &state);
    if (live) {
        (void)printf("@@LIVE_RESET {\"reason\":\"authoritative post-commit PFS3 map\"}\n");
        (void)fflush(stdout);
    }
    (void)printf("PFS3 %s completed; committed %" PRIu64 " KiB.\n",
...
fail:
    (void)fprintf(stderr, "%s: %s\n", PROG,
                  error[0] != '\0' ? error : "PFS3 transaction failed");
    journal_free(&state);
    return 1;
}
