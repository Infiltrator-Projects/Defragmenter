                            ...
                            char *error, size_t error_size)
{
    ...
    if (resolved == NULL)
        worker_error(error, error_size,
                     "cannot resolve Minix target %s: %s",
                     path, strerror(errno));
    return resolved;
}
                           ...
                           char *error, size_t error_size)
{
    ...
    if (stat(path, &status) != 0) {
        worker_error(error, error_size,
                     "cannot stat Minix target: %s", strerror(errno));
        return -1;
    }
    if (!S_ISBLK(status.st_mode) && !S_ISREG(status.st_mode)) {
        worker_error(error, error_size,
                     "Minix target is not a block device or regular image");
        return -1;
    }
    ...
    if (*size == 0U) {
        worker_error(error, error_size,
                     "cannot determine Minix target size");
        return -1;
    }
                                 ...
                                 char *error, size_t error_size)
{
    ...
    int result =
        target_identity(canonical, &identity, &physical_bytes,
                        error, error_size);
    if (result == 0 &&
