...
#define EXFAT_MAX_DIRECTORY_DEPTH 256U
...
int exfat_scan(const char *path, bool allow_dirty, ExfatVolume *v,
    ...
    if (parse_directory(v, c, 2U, "/", &c->root_clusters, root,
                        (size_t)root_len, visited, 0U, error) != 0) goto fail;
    free(visited);
    visited = NULL;
    free(root);
    root = NULL;
