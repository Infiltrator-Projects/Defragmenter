...
- [ ] **APFS** — exact spaceman-backed allocation/fragmentation analysis is complete for the bounded single-volume checkpoint/direct-CIB/flat-tree subset; recoverable offline Defragment/Growth Defrag/Recover and the first-party Test Media creator remain to complete the item.
...
- [ ] **Test Media completeness** — remove the remaining manual/reserved APFS slot only after its first-party creator and verification fixture exist.
