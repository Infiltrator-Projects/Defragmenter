...
- [ ] **UFS1/UFS2** — make UFS1 allocation mapping exact, decode file fragmentation for both supported variants, then qualify the in-tree bounded native Defragment/Growth Defrag/Recover path. Until that evidence is complete, the installed UFS worker hard-refuses all mutation verbs rather than relying only on GUI capability hiding.
...
- [ ] **GTK compatibility migration** — native mapper/operation-dispatch/privileged-helper services are the only installed control plane and GTK now consumes the native manifest directly with no installed Python capability registry; remove the remaining source-only legacy plugin registry/contracts and parity-only dispatcher modules so Python is presentation/glue only.
