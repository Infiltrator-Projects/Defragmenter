...
- [ ] **ZFS/OpenZFS member** — structured native parsing now walks all four leaf-vdev labels and their uberblock rings, selects the newest plausible TXG, decodes the MOS root block pointer, decodes the packed XDR label topology and explicitly qualifies only a single-disk top-vdev geometry before any future MOS dereference. Remaining work is MOS/dnode traversal, metaslab-array and space-map replay for exact allocation, dataset/ZPL extent traversal for fragmentation, then a recoverable offline writer contract only where pool/member semantics make deterministic safe mutation provable.
