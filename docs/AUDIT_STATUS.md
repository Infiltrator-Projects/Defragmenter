...
Applies to: release version 1.8.0-189
Audited source commit: eca4c69191720abd9e3738e2ce0995248773263e
...
Version 1.8.0-189 is the current audited release line. It retains the exact Common 1.19.10 pin, all filesystem/write-safety semantics from 1.8.0-188, the retry-safe publication workflow, GTK/GLib/X11 identity repair and complete PNG structural validation. The audited source baseline at `eca4c69191720abd9e3738e2ce0995248773263e` changes branding only: Defragmenter now uses the existing Software Centre `disk` identity already assigned to it by the Infiltrator APT catalogue, rendered in the same graphite/cyan desktop-family treatment and distributed through the existing 96×96 hicolor/private/Mint icon paths. No filesystem engine, target-safety decision, placement, mutation or recovery semantic changed for this release. Any later change beneath audited production/build/package paths requires a new source audit baseline before release.
