...
APFS exact-analysis qualification uses an independently constructed Fletcher-valid container rather than a mounted host filesystem. The fixture carries block-zero and active checkpoint NX superblocks, a checkpoint map resolving the ephemeral spaceman object, a direct chunk-info block with allocation bitmap, container and volume object maps, an unencrypted snapshot-free APFS volume and a flat catalog containing one deliberately fragmented regular-file data stream. The production worker derives every free/used block from the active spaceman bitmap and file fragmentation from catalog extent continuity, then rejects deliberately inconsistent bitmap ownership, a second volume and a sparse extent. The current APFS evidence is analysis-only: mutation/recovery is not claimed until its separate write contract and Test Media creator are qualified.
