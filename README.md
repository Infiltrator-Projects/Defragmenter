...
The graphical dashboard direction is captured in [`docs/UI_VISION.md`](docs/UI_VISION.md), including the approved concept image. The current second pass uses a wider visual dashboard, selected-volume hero surface, radial allocation summaries, functional graphical navigation, large operation cards and a locality-preserving pixel allocation image rather than the former grid or row-stripe renderer. Test Media and Settings are directly accessible from the navigation rail. Supported write actions on mounted physical volumes now offer an explicit safe-unmount-and-continue interaction while the native writer boundary continues to refuse mounted targets.
